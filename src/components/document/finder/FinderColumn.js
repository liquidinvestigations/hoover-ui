...
export default function FinderColumn({ items, pathname, prevPage, nextPage, active, selected }) {
    ...
    return (
        <List dense disablePadding className={classes.column}>
            {prevPageState && (
                <ListItem component={ButtonBase} onClick={prevPageLoading ? null : handlePrevPage}>
                    {prevPageLoading ? (
                        <CircularProgress size={18} thickness={5} className={classes.loading} />
                    ) : (
                        <ListItemIcon>{reactIcons.moreFiles}</ListItemIcon>
                    )}
                </ListItem>
            )}

            {itemsState.map((item) => (
                <FinderItem key={item.id} item={item} active={active} selected={selected} />
            ))}

            {nextPageState && (
                <ListItem component={ButtonBase} onClick={nextPageLoading ? null : handleNextPage}>
                    {nextPageLoading ? (
                        <CircularProgress size={18} thickness={5} className={classes.loading} />
                    ) : (
                        <ListItemIcon>{reactIcons.moreFiles}</ListItemIcon>
